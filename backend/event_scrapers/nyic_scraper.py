...
Scrapes immigration-related events from NYIC (New York Immigration Coalition)
NYIC is an umbrella policy and advocacy organization for over 200 groups
in New York State that work with immigrants and refugees.

Website: https://www.nyic.org/resources-training/events/
"""
...
def scrape_nyic_events(max_events=50):
    """
    Scrape events from New York Immigration Coalition website
    ...
    """
    print("[NYIC Scraper] Starting to scrape events...")

    url = "https://www.nyic.org/resources-training/events/"
    headers = {
    ...
    try:
        ...
        print(f"[NYIC Scraper] Error fetching page: {e}")
        return []
    ...
    print(f"[NYIC Scraper] Found {len(event_items)} potential event items")

    for idx, item in enumerate(event_items[:max_events]):
        try:
            event_data = parse_nyic_event(item)
            if event_data:
                normalized = normalize_event_data(event_data)
                events.append(normalized)
                print(f"[NYIC Scraper] Scraped event {idx + 1}: {normalized['title']}")
        except Exception as e:
            print(f"[NYIC Scraper] Error parsing event {idx + 1}: {e}")
            continue

    print(f"[NYIC Scraper] Successfully scraped {len(events)} events")
    return events


def parse_nyic_event(event_item):
    """
    Parse individual NYIC event item
    ...
    # Default to New York if no location found (NYIC is NY-based)
    if not event_data.get('location'):
        event_data['location'] = 'New York, NY'
        event_data['city'] = 'New York'
        event_data['state'] = 'New York'
    ...
    if desc_elem:
        ...
        event_data['description'] = f"Event hosted by New York Immigration Coalition (NYIC)"
    ...
    if link_elem:
        href = link_elem['href']
        if href.startswith('/'):
            event_data['external_url'] = f"https://www.nyic.org{href}"
        elif href.startswith('http'):
            event_data['external_url'] = href
        else:
            event_data['external_url'] = f"https://www.nyic.org/events/{href}"
    ...
    if img_elem:
        ...
        if img_src:
            if img_src.startswith('//'):
                ...
                event_data['image_url'] = f"https://www.nyic.org{img_src}"
            elif img_src.startswith('http'):
                event_data['image_url'] = img_src

    # Set timezone (NYIC is in New York)
    event_data['timezone'] = 'EST'
...
if __name__ == '__main__':
    # Test the scraper
    print("Testing NYIC Event Scraper...")
    events = scrape_nyic_events(max_events=10)
